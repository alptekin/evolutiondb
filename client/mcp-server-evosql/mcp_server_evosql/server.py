...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "structured data: each message's date, sender, subject, and a "
            "handled/open STATUS, plus per-sender totals. Call this when the user "
            "asks for an email summary, who emailed them, what still needs them, "
            "or a catch-up after being away. `since` accepts an ISO date "
            "(YYYY-MM-DD) or a keyword: today, yesterday, this_week, last_week, "
            "last_month. `until` is an optional ISO date (defaults to now).\n"
            "Per message `status` is one of: replied_by_me (I answered) | "
            "replied_by_colleague (a teammate answered in the same thread) | "
            "handled_elsewhere (same person was contacted on Teams/chat, or a "
            "later message mentions a call/Zoom/WhatsApp resolution) | open (still "
            "needs the user). Each message also has handled (bool), maybe (bool), "
            "by_whom, via_channel, and an `evidence` dict (tier/by/channel/at/"
            "phrase/note) explaining WHY. `maybe`=true marks a handled call that "
            "leaned on a HEURISTIC and wants confirmation — the cross-channel / "
            "off-channel tiers, and Outlook colleague replies (matched by subject, "
            "no real thread id). A Gmail colleague reply and my own reply are "
            "confident (maybe=false). When rendering: filter the nag / 'what still "
            "needs me' view to status=='open'; for handled rows show the evidence "
            "`note` as a muted line, and present maybe=true rows as 'likely "
