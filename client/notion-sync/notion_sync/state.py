...
WATERMARK_KEY = "notion_state_latest"
...
class MemoryStore:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        try:
            ...
            raise RuntimeError(
                "evolutiondb-notion-sync requires psycopg. Install "
        ...
        for attempt in range(self._RECONNECT_ATTEMPTS):
            try:
                ...
                if not self._is_dead(exc):
                    raise
                last = exc
                print(f"[notion-sync] db connection lost "
    ...
    def get_watermark_iso(self) -> Optional[str]:
        def run(cur):
            ...
            if not rows:
                return None
            v = _parse_value(rows[0][0]).get("last_edited_time")
            return str(v) if v else None
        return self._retry(run)

    def set_watermark_iso(self, iso: str) -> None:
        value = json.dumps({"last_edited_time": iso,
                              "saved_at":        time.time()})
