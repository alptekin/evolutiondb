"""Memory backend for gmail-sync. Same template as the sibling
connector packages."""
...
WATERMARK_KEY = "gmail_state_latest"
...
class MemoryStore:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        try:
            ...
            raise RuntimeError(
                "evolutiondb-gmail-sync requires psycopg. Install with "
        ...
        for attempt in range(self._RECONNECT_ATTEMPTS):
            try:
                ...
                if not self._is_dead(exc):
                    raise
                last = exc
                print(f"[gmail-sync] db connection lost "
    ...
    # ---------------------------------------------------------------- #
    #  Watermark — single internalDate-ish unix-millis epoch            #
    # ---------------------------------------------------------------- #
    def get_watermark_ms(self) -> Optional[int]:
        def run(cur):
            cur.execute(
            ...
            if not rows:
                return None
            v = _parse_value(rows[0][0]).get("internal_date_ms")
            return int(v) if v is not None else None
        return self._retry(run)

    def set_watermark_ms(self, internal_date_ms: int) -> None:
        value = json.dumps({"internal_date_ms": int(internal_date_ms),
                            "saved_at":        time.time()})
