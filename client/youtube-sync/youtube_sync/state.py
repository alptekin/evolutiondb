...
class MemoryStore:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        try:
            ...
            raise RuntimeError(
                "evolutiondb-youtube-sync requires psycopg. Install "
        ...
        for attempt in range(self._RECONNECT_ATTEMPTS):
            try:
                ...
                if not self._is_dead(exc):
                    raise
                last = exc
                print(f"[youtube-sync] db connection lost "
    ...
    # Per-resource watermark.
    def get_watermark_iso(self, resource: str) -> Optional[str]:
        key = f"youtube_state_{resource}"
        ...
        def run(cur):
            ...
            if not rows:
                return None
            v = _parse_value(rows[0][0]).get("synced_at")
            return str(v) if v else None
        return self._retry(run)

    def set_watermark_iso(self, resource: str, iso: str) -> None:
        key = f"youtube_state_{resource}"
        value = json.dumps({"synced_at": iso,
                              "saved_at": time.time()})
