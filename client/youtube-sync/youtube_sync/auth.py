"""
Google OAuth 2.0 loopback-redirect flow for the YouTube Data API,
using the same `Installed app` client gmail-sync uses. Scope is
`youtube.readonly` only; cache lives at
`~/.evosql/youtube_token.json` so it does not collide with the
Gmail / Calendar caches.
"""
...
class _Handler(BaseHTTPRequestHandler):
    server_version = "evolutiondb-youtube-sync/0.1"
    captured: Dict = {}
    ...
    def do_GET(self):
        ...
        body = (b"<!doctype html><html><body>"
                ...
                b"<p>evolutiondb-youtube-sync got the code.</p>"
                b"</body></html>")
        if "error" in params:
...
def _post(url: str, form: Dict[str, str]) -> Dict:
    ...
    try:
        ...
        raise AuthError(
            ...
            f"{payload.get('error', '')} — "
            f"{payload.get('error_description', '')}") from None
...
class YouTubeAuth:
    def __init__(self, client_id: str, client_secret: str,
        ...
        self.cache         = TokenCache(Path(cache_path), connector="youtube",
                                        namespace=namespace)
    ...
    def ensure_token(self, interactive: bool = False) -> str:
        ...
        if cached.get("refresh_token"):
            try:
                ...
                if not interactive:
                    raise
                print(f"[youtube-sync] refresh failed ({exc}); "
    ...
    def _interactive(self) -> Dict:
        ...
        print(f"[youtube-sync] opening browser for consent …\n  {url}",
              file=sys.stderr, flush=True)
        try:
            webbrowser.open(url)
        except Exception:
