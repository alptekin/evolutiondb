"""
Memory backend — writes Azure DevOps records and per-PR watermarks into
EvolutionDB over the PG wire protocol. Same pattern as github-sync/teams-sync;
copied rather than factored into a shared library so each connector package
stays a single `pip install` away from working.
"""
...
class MemoryStore:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        try:
            ...
            raise RuntimeError(
                "evolutiondb-azure-devops-sync requires psycopg. Install with "
        ...
        for attempt in range(self._RECONNECT_ATTEMPTS):
            try:
                ...
                if not self._is_dead(exc):
                    raise
                last = exc
                print(f"[azure-devops-sync] db connection lost "
    ...
    # ---------------------------------------------------------------- #
    #  Watermarks — one row per project/repo#pr key                     #
    # ---------------------------------------------------------------- #
