...
WATERMARK_KEY = "imessage_state_latest"
...
class MemoryStore:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        try:
            ...
            raise RuntimeError(
                "evolutiondb-imessage-sync requires psycopg. Install "
        ...
        for attempt in range(self._RECONNECT_ATTEMPTS):
            try:
                ...
                if not self._is_dead(exc):
                    raise
                last = exc
                print(f"[imessage-sync] db connection lost "
    ...
    # ---------- watermark (max apple-date integer) ----------
    def get_watermark(self) -> Optional[int]:
        def run(cur):
            ...
            if not rows:
                return None
            v = _parse_value(rows[0][0]).get("apple_date_ns")
            return int(v) if v is not None else None
        return self._retry(run)

    def set_watermark(self, apple_date_ns: int) -> None:
        value = json.dumps({"apple_date_ns": int(apple_date_ns),
                              "saved_at":       time.time()})
