...
class Config:
    def __init__(self):
        ...
        # back to the SLACK_USER_TOKEN env var (single-user / dev / seed).
        self.token       = _resolve_token("slack", self.user_id,
                                           "SLACK_USER_TOKEN")
...
def _resolve_token(connector: str, namespace: str, env_var: str) -> str:
    ...
    any store error falls back to the env var so a misconfig never breaks sync."""
    try:
