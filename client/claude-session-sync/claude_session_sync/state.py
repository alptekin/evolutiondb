"""
Memory backend — writes Claude-session records and per-session watermarks into
EvolutionDB over the PG wire protocol. Same pattern as github-sync; copied so
the connector stays a single `pip install` away from working.
"""
...
WATERMARK_PREFIX = "claude_session_state_"
...
class MemoryStore:
    ...
    def __init__(self, host: str, port: int, user: str, password: str,
        ...
        try:
            ...
            raise RuntimeError(
                "evolutiondb-claude-session-sync requires psycopg. Install "
                "with `pip install psycopg[binary]>=3.1`."
            ) from exc
        ...
        for attempt in range(self._RECONNECT_ATTEMPTS):
            try:
                ...
                if not self._is_dead(exc):
                    raise
                last = exc
                print(f"[claude-session-sync] db connection lost "
    ...
    # ---------------------------------------------------------------- #
    #  Watermarks — one row per session id                              #
    # ---------------------------------------------------------------- #
    def get_watermark(self, session_id: str) -> Optional[str]:
        key = f"{WATERMARK_PREFIX}{session_id}"
    ...
    def set_watermark(self, session_id: str, updated_at: str) -> None:
        key = f"{WATERMARK_PREFIX}{session_id}"
        value = json.dumps({"updated_at": updated_at, "saved_at": time.time()})
